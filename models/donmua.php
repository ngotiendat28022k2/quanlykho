...
class DonMua
{
    ...
    {
        $db = DB::getInstance();
        // Thay vì chèn trực tiếp $id vào câu truy vấn SQL, bạn sử dụng tham số chuẩn
        $req = $db->prepare('SELECT db.Id, db.NgayMua, nv.TaiKhoan, kh.TenNCC, db.ThanhTien, db.TrangThai 
                             FROM DonMua db 
                             JOIN NhanVien nv ON nv.Id = db.IdNV 
                             JOIN NhaCungCap kh ON kh.Id = db.IdNCC 
                             WHERE db.Id = :id');
        // Truyền giá trị của $id qua tham số
